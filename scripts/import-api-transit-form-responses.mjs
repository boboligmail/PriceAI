...
function buildApinodeEvidenceRows({ response, parsed }) {
  ...
  const station = {
    ...
    summary:
      "APINode 由站长通过 ai-home 表单提交，当前公开资料包含 Sub2API 分组倍率截图和监控截图；ai-home 尚未接入测试 Key 做真实调用检测。",
    ...
    availability_note: "站长截图包含 30 日监控样本；尚未接入 ai-home 自测，暂不写入近 7 日可用率。",
    feedback_pending_count: 0,
    feedback_verified_risk_count: 0,
    feedback_merchant_responded_count: 0,
    feedback_main_themes: [],
    feedback_public_notes: "注册账户输入 ai-home 首次充值后可额外获得 5.88 通用额度。",
  ...
  const run = {
    ...
    error_message: "站长公开截图已解析；缺少机器可读价格接口和 ai-home 自测 Key。",
...
function buildManualOffer({ stationId, family, standardModel, rawModelName, groupName, multiplier, accountPool, channelType, collectedAt, monitor }) {
  return {
    ...
    availability_first_checked_at: null,
    availability_last_checked_at: null,
    availability_note: `站长监控截图显示 30 日样本：${monitor.label} ${formatPercent(monitor.availability30d)} / ${monitor.sampleCount} 条；尚未接入 ai-home 自测。`,

...
const headers = {
  "user-agent":
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Safari/537.36 ai-home-price-collector/1.0",
...
function normalizeCloudGpuRow(row, index) {
  ...
  const acceleratorCount = Number(row.num_accelerators_float) || Number(row.num_accelerators_numerator) || 1;
